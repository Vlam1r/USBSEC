...
/**
 * @brief Set up the USB controller in host mode, clearing any previous state.
 *
 */
void usb_host_init() {
    // Reset usb controller
    ...
    // Clear any previous state in dpram just in case
    memset(usbh_dpram, 0, sizeof(*usbh_dpram)); // <1>
    ...
    // Enable the USB controller in host mode.
    usb_hw->main_ctrl = USB_MAIN_CTRL_CONTROLLER_EN_BITS | USB_MAIN_CTRL_HOST_NDEVICE_BITS;

    // Setup keepalive
    usb_hw->sie_ctrl = USB_SIE_CTRL_KEEP_ALIVE_EN_BITS |
            USB_SIE_CTRL_SOF_EN_BITS; // <2>
 ...
 * @return false
 */
static inline bool ep_is_tx(usb_endpoint_configuration *ep) {
...
void usb_handle_setup_packet(void) {
    ...
    spi_send_blocking(usb_dpram->setup_packet, 8, 1);
    /*
 ...
 */
static void usb_handle_ep_buff_done(usb_endpoint_configuration *ep) {
...
void ep1_out_handler(uint8_t *buf, uint16_t len) {
    ...
    usb_endpoint_configuration *ep = usb_get_endpoint_configuration(EP2_IN_ADDR);
